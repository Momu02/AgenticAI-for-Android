...
>网上关于主动式对话AI这个项目，能简单理解和实现的，很少。对于一个普通人（不懂代码）来说，AI最大的用处就是聊天。然而截至2025年，网上似乎依然没有什么真正意义上的AI聊天工具。为什么这样说？因为现在所有的AI基本都只是一个问答工具。这对于孤独的我来说，网上的很多桌宠、助手都没法完全让我满意——它们不是朋友，它们只是冷冰冰的问答工具。于是我就打算自己实现这个想法。奈何本人技术力也不高，而且时代发展日新月异（知识在飞快的迭代）......额好吧，其实就是懒得深入研究。总之，我通过“曲线救国”的方式也基本实现了我想要的真正意义上能聊天的AI。
...
为什么用Autojs？
>Autojs作为安卓老牌自动化脚本软件，使用和脚本参考都比较丰富，而且AI可以直接编辑该脚本（没错，它可以边执行边自己优化）。
>然而，目前Autojs也有一个难解的问题。即使通过shell执行脚本后，它也不能在没有后台的情况下打开别的应用（脚本里尝试了三种方法都不行）。所以需要习惯将Autojs常驻后台。不过也有其他选择，我个人长期常驻FV悬浮球，所以我可以利用FV悬浮球来打开软件。（该部分代码也写入了脚本）

### 一.下载并配置Operit AI（或其他对接API且大模型有手机管理权限的应用）
